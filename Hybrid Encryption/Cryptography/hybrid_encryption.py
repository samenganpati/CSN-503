...
default_path = f"C:/Users/{getuser()}/Hybrid Encryption/CipherText"
...
def encryption(file_path):
    ...
    try:
        ...
        mgs = f"File successfully encrypted and stored at below location :\n{default_path}"
...
def decryption(file_path):
    ...
    if not (RSA_keys.is_file_exist() and AES_key.is_AES_key_exist()):
        mgs = "Keys which are used for encryption either deleted or corrupted, Cann't Decrypt File "
        return mgs
    ...
    try:
        ...
        if not is_file_encrypted(magic, cipher_text):
            mgs = "Given file is not encrypted, please encrypt it first then come"
            return mgs
        ...
        mgs = f"File successfully decrypted and stored at below location :\n{default_path}"

    except Exception as e:
        mgs = "There is some error please check the selected file 🤦‍♂️"
        print(e)
