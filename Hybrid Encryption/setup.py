...
def message(name, button, mgs_label):
    ...
    if button["text"] == "Encrypt The File":
        mgs = encryption(name)
        mgs_label.config(text=mgs)

    if button["text"] == "Decrypt The File":
        mgs = decryption(name)
        mgs_label.config(text=mgs)
...
def open_file():
    ...
    if name:
        ...
        encrypt_button = ttk.Button(root, text="Encrypt The File", command=lambda: message(name, encrypt_button, mgs_label))
        decrypt_button = ttk.Button(root, text="Decrypt The File", command=lambda: message(name, decrypt_button, mgs_label))
        encrypt_button.place(x=110, y=100)
        decrypt_button.place(x=540, y=100)
...
title_label = ttk.Label(root, text="Welcome to Hybrid Encryption Application Using RSA", font=("Lucida Console", 14,"bold"))
title_label.pack()
...
label = ttk.Label(root, text="No File Selected")  # Label to display the name of selected file.
label.pack()
